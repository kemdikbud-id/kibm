 ...
 */
class Proposal_Startup extends Frontend_Controller
{
	...
	public function create()
	{
		...
		if ($this->input->post('mode') == 'add')
		{
			...
			if ($user == NULL)
			{
				...
				$user->program_id		= PROGRAM_STARTUP;
			...
			if ($this->anggota_model->is_sudah_terdaftar($mahasiswa_id, $kegiatan->id))
			{
				$this->session->set_flashdata('result', [
					'page_title' => 'Tambah Usulan Startup Baru',
					'message' => 'Mahasiswa ini sudah didaftarkan.',
					'link_1' => anchor('proposal-startup/create', 'Kembali')
				]);
			...
			if ($this->db->trans_status() === TRUE)
			{
				...
				$this->session->set_flashdata('result', [
					...
					'link_1' => anchor('proposal-startup/index', 'Kembali ke Daftar Startup')
				]);
	...
	public function update($proposal_id)
	{
		...
		if ($this->input->method() == 'post')
		{
			...
			$this->session->set_flashdata('result', [
				'page_title' => 'Edit Info Startup',
				'message' => 'Berhasil di update',
				'link_1' => anchor('proposal-startup/update/' . $proposal_id, 'Kembali'),
				'link_2' => anchor('proposal-startup/index/', 'Kembali ke Daftar Startup')
			]);
	...
	public function delete($proposal_id)
	{
		if ($this->input->method() == 'post')
		{
			...
			if ($this->db->trans_status() === TRUE)
			{
				...
				$this->session->set_flashdata('result', array(
					'page_title' => 'Hapus Usulan Startup',
					'message' => 'Usulan Startup berhasil dihapus',
					'link_1' => '<a href="'.site_url('proposal-startup/index').'">Kembali</a>'
				));
	...
	public function cancel_submit($proposal_id)
	{
		...
		redirect('proposal-startup/index');
	}
}
